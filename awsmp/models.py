...
from .constants import CATEGORIES, TYPE
...
class PromotionalResourcesModel(BaseModel):
    """
    Model for promotional resource details
    from entity details
    """
    ...
    def additional_resources_validator(cls, value) -> SupportResources:
        return [{"Text": resource["Text"], "Url": HttpUrl(resource["Url"])} for resource in value]


class SupportInformationModel(BaseModel):
    """
    Model for support information details
    from entity details
    """
...
class RegionAvailabilityModel(BaseModel):
    """
    Model for region availability
    inforation details from entity
    details
    """
...
class DiffAddedModel(BaseModel):
    """
    Model for fields that have been added in a diff
    comparison
    """
...
class DiffRemovedModel(BaseModel):
    """
    Model for fields that have been removed in a
    diff comparison
    """
...
class DiffChangedModel(BaseModel):
    """
    Model for fields that have been changed in a
    diff comparison
    """
...
class EntityModel(BaseModel):
    ...
    def get_diff_model_type(
        ...
        """
        Determine the type of diff by comparing values in the field between
        the Marketplace listing and the local config
    ...
    def _get_diff_model(self, local_entity: EntityModel) -> DiffModel:
        ...
        for entity_key, entity_value in local_entity.dict().items():
            if entity_key not in non_model_fields:
                for model_key, model_value in entity_value.items():
                    res = EntityModel.get_diff_model_type(model_key, self.dict()[entity_key][model_key], model_value)
                    EntityModel.add_to_diff_list(res, diff_added, diff_removed, diff_changed)
            else:
                res = EntityModel.get_diff_model_type(entity_key, self.dict()[entity_key], entity_value)
                EntityModel.add_to_diff_list(res, diff_added, diff_removed, diff_changed)
