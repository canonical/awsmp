...
class TestVersion:
    ...
    def test_version_ami_id(self):
        model = models.Version(**self._get_version_details())
...
class TestInstanceTypePricing:
    @pytest.mark.parametrize(
        "instance_type_and_pricing,expected",
        [
            ({"name": "c1.medium", "hourly": 0.004, "yearly": 24.528}, "0.004"),
            ({"name": "c3.medium", "hourly": 0.012, "monthly": 57.528}, "0.012"),
            ({"name": "c1.metal", "hourly": 0, "yearly": 0}, "0"),
        ],
    )
    def test_instance_type_pricing(self, instance_type_and_pricing, expected):
        model = models.InstanceTypePricing(**instance_type_and_pricing)
        assert str(model.price_hourly) == expected

    def test_instance_type_with_monthly_and_yearly(self):
    ...
    def test_instance_type_without_monthly_and_yearly(self):
        instance_type_and_pricing: dict[str, Any] = {
            "name": "c1.medium",
            "hourly": 0.004,
            "yearly": 24.528,
...
class TestOffer:
    def _get_offer_details(self):
        return {
            ...
            "instance_types": [
                {"name": "c3.medium", "hourly": 0.012, "monthly": 57.528},
                {"name": "c4.large", "hourly": 0.078, "monthly": 123.456},
            ],
    ...
    def test_instance_type_and_pricing_from_offer(self):
        model = models.Offer(**self._get_offer_details())
        assert model.instance_types[1].name == "c4.large" and str(model.instance_types[1].price_monthly) == "123.456"

    def test_invalid_instance_type_and_pricing_from_offer_with_yearly(self):
        offer_detail = self._get_offer_details()
        offer_detail["instance_types"][0]["yearly"] = 123.456
        with pytest.raises(ValidationError):
            models.Offer(**offer_detail)
